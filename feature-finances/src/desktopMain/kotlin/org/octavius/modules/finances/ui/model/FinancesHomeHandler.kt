...
class FinancesHomeHandler(
    ...
    private fun getSql(): String {
        val totalBalanceSubquery = dataAccess.select("COALESCE(SUM(s.amount), 0)")
            ...
            .where("a.type IN ('ASSET', 'LIABILITY')").toSql()
        ...
        val expensesMonthSubquery = dataAccess.select("COALESCE(SUM(s.amount), 0)")
            ...
            .where("a.type = 'EXPENSE' AND t.transaction_date >= date_trunc('month', now())").toSql()

        val incomeMonthSubquery = dataAccess.select("ABS(COALESCE(SUM(s.amount), 0))")
            .from("finances.splits s JOIN finances.accounts a ON a.id = s.account_id JOIN finances.transactions t ON t.id = s.transaction_id")
            .where("a.type = 'INCOME' AND t.transaction_date >= date_trunc('month', now())").toSql()

        val recentTransactionsInner =
            dataAccess.select("t.id, t.transaction_date as date, t.description, COALESCE(ABS(SUM(s.amount) FILTER (WHERE s.amount > 0)), 0) as amount")
                .from("finances.transactions t JOIN finances.splits s ON s.transaction_id = t.id")
                .groupBy("t.id, t.transaction_date, t.description").orderBy("t.transaction_date DESC").limit(5).toSql()

        val recentTransactionsSubquery =
            dataAccess.select("COALESCE(array_agg(dynamic_dto('finances_dashboard_transaction', jsonb_build_object('id', id, 'date', date, 'description', description, 'amount', amount))), '{}')")
                .fromSubquery(recentTransactionsInner).toSql()
        ...
        val topAccountsInner = dataAccess.select("a.id, a.name, COALESCE(SUM(s.amount), 0) as balance")
            ...
            .where("a.type IN ('ASSET', 'LIABILITY')").groupBy("a.id, a.name")
            .orderBy("ABS(COALESCE(SUM(s.amount), 0)) DESC").limit(5).toSql()

        val topAccountsSubquery =
            dataAccess.select("COALESCE(array_agg(dynamic_dto('finances_dashboard_account', jsonb_build_object('id', id, 'name', name, 'balance', balance))), '{}')")
                .fromSubquery(topAccountsInner).toSql()
    ...
    fun loadData() {
        scope.launch {
            ...
            val result = withContext(Dispatchers.IO) {
                dataAccess.select(sql).toSingleOf<DashboardData>()
            }
            when (result) {
