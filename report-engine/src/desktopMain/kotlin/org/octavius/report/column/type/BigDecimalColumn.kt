...
/**
 * Kolumna do wyświetlania liczb o wysokiej precyzji (BigDecimal) w raporcie.
 * Obsługuje filtrowanie numeryczne i prawidłowe formatowanie.
 *
 ...
 * @param formatter Funkcja formatowania liczb do wyświetlenia (domyślnie toPlainString, aby uniknąć notacji naukowej)
 */
class BigDecimalColumn(
    ...
    private val formatter: (BigDecimal?) -> String = { it?.toPlainString() ?: "" }
) : ReportColumn(header, ColumnWidth.Flexible(width), filterable, sortable) {

    override fun createFilter(): Filter<*> {
        return NumberFilter(BigDecimal::class) { it.replace(",", ".").toBigDecimalOrNull() }
    }

    @Composable
    override fun RenderCell(item: Any?, modifier: Modifier) {
        val value = item as? BigDecimal
