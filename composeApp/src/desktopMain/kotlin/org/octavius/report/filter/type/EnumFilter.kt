...
{
    override fun createDefaultData(): EnumFilterData<E> {
        return EnumFilterData(enumClass)
    }

    @Composable
    override fun RenderFilterUI(data: EnumFilterData<E>) {
        ...
        Row(
            ...
            RadioButton(
                selected = data.include.value,
                onClick = { data.include.value = true }
            )
            Text(Translations.get("filter.enum.includeSelected"), modifier = Modifier.padding(end = 8.dp))

            RadioButton(
                selected = !data.include.value,
                onClick = { data.include.value = false }
            )
            ...
            Row(
                ...
                Checkbox(
                    ...
                    onCheckedChange = { checked ->
                        if (checked) {
                            data.values.add(enumValue)
                        } else {
                            data.values.remove(enumValue)
                        }
                    }
                )
    ...
    override fun buildBaseQueryFragment(
        ...
        val include = data.include.value
